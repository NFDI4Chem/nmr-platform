...
deploy_service() {
    ...
    if [ "$(docker pull "$APP_IMAGE" | grep -c "Status: Image is up to date")" -eq 0 ]; then
        echo "📦 New image available for app/worker."
        ...
        # Wait for both new containers to be healthy
        for service in app worker; do
            NEW_CONTAINER_ID=$(docker ps --filter "name=${service}" --format "{{.ID}}" | head -n 1)
            echo "🔍 New $service container ID: $NEW_CONTAINER_ID"
        ...
        echo "✅ No update for app/worker. Skipping deployment."
    fi
}
