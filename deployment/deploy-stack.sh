#!/bin/bash
# Full Stack Deployment Script for NMR Platform (Application + Storage)

set -euo pipefail

# Configuration
readonly SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly MAIN_DEPLOY_SCRIPT="$SCRIPT_DIR/deploy-app.sh"
readonly STORAGE_DEPLOY_SCRIPT="$SCRIPT_DIR/deploy-storage.sh"
...
check_scripts() {
    ...
    [[ -f "$STORAGE_DEPLOY_SCRIPT" ]] || error "Storage deployment script not found: $STORAGE_DEPLOY_SCRIPT"
    [[ -x "$MAIN_DEPLOY_SCRIPT" ]] || error "Main deployment script is not executable: $MAIN_DEPLOY_SCRIPT"
    [[ -x "$STORAGE_DEPLOY_SCRIPT" ]] || error "Storage deployment script is not executable: $STORAGE_DEPLOY_SCRIPT"
}

# Deploy Storage (MinIO)
deploy_storage() {
    log "🗄️  Deploying Storage (MinIO)..."
    if "$STORAGE_DEPLOY_SCRIPT" deploy; then
...
# Stop Storage (MinIO)
stop_storage() {
    log "🛑 Stopping Storage (MinIO)..."
    if "$STORAGE_DEPLOY_SCRIPT" stop; then
        success "Storage stopped"
    else
        warning "Failed to stop Storage (it may not be running)"
    fi
}
...
status_all() {
    ...
    log "Storage Status:"
    "$STORAGE_DEPLOY_SCRIPT" status || true
    echo ""
...
logs_all() {
    ...
    if [[ "$service" == "storage" ]] || [[ "$service" == "minio" ]]; then
        log "📋 Storage Logs:"
        "$STORAGE_DEPLOY_SCRIPT" logs
...
show_usage() {
    echo "Usage: $0 {deploy|deploy-storage|deploy-app|stop|stop-all|restart|status|logs|health|auto-deploy|watch|backup}"
    echo ""
    echo "🚀 Deployment Commands:"
    echo "  deploy         - Deploy both Storage and main application (default)"
    echo "  deploy-storage - Deploy only Storage (MinIO)"
    ...
    echo "  logs [service] - Show logs (service: storage|app|all)"
